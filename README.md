...
主界面现为 **Swing 原生桌面窗口**，Windows 和 macOS 均使用系统控件显示投递配置、状态、记录、AI 话术、卡密与套餐。启动时不再打开本地网页或 Chrome 应用模式。后端仍只在 `127.0.0.1` 提供 API，桌面窗口调用本机 API；执行投递时才打开独立的 Chrome 浏览器窗口，供招聘网站登录与自动化操作。无头运行或 CI 可设 `jobpilot.console=false` 跳过窗口。
