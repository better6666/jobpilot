...
| POST | `/admin/ai/models` | 拉中转站的模型列表，可带临时覆盖值；响应只回模型名，不回 key |
...
`manage.html` 的模型名旁边有个「拉取模型」：用输入框里当前的地址和 Key 去中转站问一份
列表，点一下模型名直接填入。地址和 Key 不用先保存——刚敲完就能拉，请求体带临时覆盖值，
库里存的旧值不动。

各家中转站挂 `/models` 的位置和返回形状都不统一，所以 `src/lib/relay.ts` 里做了两段兜
（都有单测）：**路径**按 `地址本身/models` → 去掉尾部 `/v1` 后的根 → 同域名根 挨个试，
404 就换下一条；**形状**认 `{data:[{id}]}`、`{data:["名"]}`、`{models:[...]}`、
顶层数组，元素里认 `id`/`model`/`name` 三种键。第一条返回得了非空列表的就收工。

兜底路径可能命中同一域名下的另一套服务，所以拉到的列表要点过「测试连通」才算数，不能当免检。
